...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    val characterWord = w.filter(_.isLetter)
    val characterMap = characterWord.groupBy(_.toLower).mapValues(_.length)
    val characterList = characterMap.toList.sorted
    ...
    occurrences match {
      ...
      case (char, num) :: rest => {
        ...
        val currentCombinations = for {
          subset <- restCombinations
          count <- 1 to num
        } yield (char, count) :: subset
        
        currentCombinations ::: restCombinations
      }
    }
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    y.foldLeft(x.toMap) {
      case (occurrencesMap, (char, num)) => {
        occurrencesMap.get(char) match {
          case Some(count) if count > num => occurrencesMap.updated(char, count - num)
          case Some(count) if count == num => occurrencesMap - char
